...
class _ShoppingListScreenState extends State<ShoppingListScreen> {
  ...
  Future<void> _add() async {
    ...
    try {
      ...
      if (res.statusCode != 200) {
        throw Exception("서버 응답 오류 (${res.statusCode}): ${res.body}");
      }
    } catch (e) {
      if (!mounted) return;
      ...
      setState(() => _items.add(
        ShoppingItem(name: _nameCtrl.text, quantity: _qtyCtrl.text, unit: _unit),
      ));
    }
    _nameCtrl.clear();
    _qtyCtrl.clear();
  ...
  Future<void> _fromUrl() async {
    ...
    try {
      ...
      if (res.statusCode != 200) {
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(content: Text("API 오류: ${res.statusCode}")),
        );
        return;
      }
      ...
      if (data is List) {
        setState(() {
          _items = data.map<ShoppingItem>((e) {
            return ShoppingItem(
              name: (e['재료명'] ?? '').toString(),
              quantity: (e['최종수량'] ?? '').toString(),
              unit: (e['단위'] ?? '').toString(),
            );
          }).toList();
        });
        ...
        ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(content: Text("쇼핑 리스트 생성 완료")),
        );
      }
    } catch (e) {
      ScaffoldMessenger.of(context).showSnackBar(
          ...
          Row(mainAxisAlignment: MainAxisAlignment.spaceBetween, children: [
            ...
            TextButton(
              onPressed: () => _confirmDelete(
                  msg: "모든 항목을 삭제하시겠습니까?",
                  onConfirm: () async {
                    await http.delete(Uri.parse('$baseUrl/shopping-list-clear'));
